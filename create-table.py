...
if __name__ == "__main__":
    ...
    for key, value in final_htseq_dict.items():
        ...
        for key2, value2 in final_star_dict.items():
            if key == key2:
                htseq_values[key] += "%s:%.2f," %("feature", ((value2[0]-final_htseq_dict[key]["no_feature"]-final_htseq_dict[key]["ambiguous"]-final_htseq_dict[key]["not_aligned"]-final_htseq_dict[key]["too_low_aQual"])/(value2[0]+final_htseq_dict[key]["alignment_not_unique"]))*100)
                for name in htseq_names:
                    htseq_values[key] += "%s:%.2f," %(name, value[name]/((value2[0]+final_htseq_dict[key]["alignment_not_unique"]))*100)
                #for sub, number in value.items():
